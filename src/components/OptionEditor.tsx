...
interface Props {
  ...
  onUpdate: (id: string, field: 'text' | 'color' | 'remark', value: string) => void;
}
  ...
  return (
    <div>
      <div className="option-editor">
        {options.map((opt, idx) => (
          <div key={opt.id} className="option-item-wrapper">
            ...
            <input
              className="option-remark-input"
              value={opt.remark}
              onChange={e => onUpdate(opt.id, 'remark', e.target.value)}
              placeholder="备注说明（可选）..."
              maxLength={100}
            />
          </div>
